    ...
    return (
        <div className="page-content">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-6)', flexWrap: 'wrap', gap: 'var(--space-3)' }}>
                <div>
                    <h2 style={{ fontSize: '1.5rem', fontWeight: 700, letterSpacing: '-0.03em' }}>Notifications</h2>
                    <p style={{ fontSize: '0.875rem', color: 'var(--gray-500)', marginTop: '4px' }}>{unreadCount} unread notifications</p>
                </div>
                {unreadCount > 0 && (
                        ...
                        {notifications.map(n => {
                            ...
                            return (
                                <div key={n.id} style={{
                                    ...
                                    <div style={{ width: '36px', height: '36px', borderRadius: 'var(--radius-md)', background: style.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                        <Bell size={16} style={{ color: style.color }} />
                                    </div>
                                    <div style={{ flex: 1 }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
                                            <h4 style={{ fontWeight: n.is_read ? 500 : 600, fontSize: '0.875rem' }}>{n.title}</h4>
                                            {!n.is_read && <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: 'var(--orange-500)' }} />}
                                        </div>
